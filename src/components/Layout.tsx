...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white dark:bg-gray-900 transition-colors">
      ...
      {/* Main Header / Navbar */}
      <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 dark:bg-gray-800/95 backdrop-blur-md shadow-md' : 'bg-white dark:bg-gray-900'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-24 lg:h-32">
            <Link to="/" className="flex items-center gap-3 sm:gap-4 group">
              {logoUrl ? (
                <img src={logoUrl} alt="Doctors Home Care" className="h-16 sm:h-20 lg:h-24 w-auto" />
              ) : (
                <div className="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 bg-theme-primary rounded-xl flex items-center justify-center group-hover:opacity-90 transition-opacity shrink-0">
                  <Stethoscope className="w-7 h-7 sm:w-10 sm:h-10 lg:w-12 lg:h-12 text-white" />
                </div>
              )}
              ...
              <div className="leading-tight">
                ...
                <div className="hidden sm:block text-xs sm:text-sm lg:text-base text-theme-primary dark:text-teal-300 font-semibold mt-0.5">
                  {tagline}
                </div>
              </div>
            </Link>
              ...
              {navLinks.map((link) => (
                <Link key={link.to} to={link.to}
                  className={`px-3 xl:px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
                    isActive(link.to) ? 'bg-theme-primary-50 text-theme-primary dark:bg-gray-800 dark:text-teal-400' : 'text-gray-600 dark:text-gray-300 hover:text-theme-primary hover:bg-gray-50 dark:hover:bg-gray-800'
                  }`}>
                  {link.label}
                </Link>
              ))}
              <button onClick={toggleDarkMode} className="ml-2 p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors" aria-label="Toggle dark mode">
                {darkMode ? <Sun size={18} className="text-amber-400" /> : <Moon size={18} className="text-gray-500" />}
              </button>
            </nav>

            {/* Mobile Menu Button */}
            <button onClick={() => setMenuOpen(!menuOpen)} className="lg:hidden p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors" aria-label="Toggle menu">
              {menuOpen ? <X size={28} className="text-gray-700 dark:text-gray-200" /> : <Menu size={28} className="text-gray-700 dark:text-gray-200" />}
            </button>
          </div>
        </div>

        {/* Mobile Dropdown Menu */}
        <div className={`lg:hidden overflow-hidden transition-all duration-300 ${menuOpen ? 'max-h-96 border-t border-gray-100 dark:border-gray-800' : 'max-h-0'}`}>
          <nav className="px-4 py-3 bg-white dark:bg-gray-900 space-y-1">
            {navLinks.map((link) => (
              <Link key={link.to} to={link.to}
                className={`block px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                  isActive(link.to) ? 'bg-theme-primary-50 text-theme-primary dark:bg-gray-800 dark:text-teal-400' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-theme-primary'
                }`}>
                {link.label}
              </Link>
            ))}
            <button onClick={toggleDarkMode} className="flex items-center gap-2 w-full px-4 py-2.5 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
            ...
            <div>
              ...
              <div>
                ...
                <div className="flex items-center flex-wrap gap-2.5">
                  {settings?.facebook && (
                    <a href={settings.facebook} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-teal-900/80 hover:bg-blue-600 flex items-center justify-center text-teal-200 hover:text-white transition-all duration-200" aria-label="Facebook">
                      <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                    </a>
                  )}
                  {settings?.instagram && (
                    <a href={settings.instagram} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-teal-900/80 hover:bg-pink-600 flex items-center justify-center text-teal-200 hover:text-white transition-all duration-200" aria-label="Instagram">
                  ...
                  {settings?.whatsapp && (
                    <a href={`https://wa.me/${settings.whatsapp.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-teal-900/80 hover:bg-green-500 flex items-center justify-center text-teal-200 hover:text-white transition-all duration-200" aria-label="WhatsApp">
                      <svg className="w-4 h-4 fill-current" viewBox="0 0 32 32"><path d="M16.004 0h-.008C7.174 0 0 7.176 0 16c0 3.5 1.132 6.744 3.054 9.378L1.054 31.29l6.118-1.962C9.716 31.06 12.76 32 16.004 32 24.826 32 32 24.826 32 16S24.826 0 16.004 0zm9.31 22.606c-.39 1.1-1.932 2.014-3.164 2.28-.844.18-1.948.324-5.67-1.218-4.762-1.97-7.826-6.81-8.072-7.134-.238-.324-1.932-2.574-1.932-4.906s1.224-3.482 1.66-3.962c.39-.432.9-.566 1.224-.566.15 0 .284.008.406.014.436.018.654.044.942.726.358.846 1.228 2.99 1.334 3.21.108.216.216.504.072.81-.14.312-.264.45-.48.696-.216.246-.422.436-.638.704-.2.236-.424.49-.178.926.246.436 1.094 1.806 2.354 2.928 1.622 1.444 2.99 1.892 3.418 2.1.324.156.71.13.948-.19.3-.39.668-1.034 1.042-1.67.266-.452.602-.508 1.012-.346.416.15 2.634 1.242 3.086 1.466.452.224.752.336.864.52.11.184.11 1.06-.28 2.16z"/></svg>
                    </a>
                  )}
                  {viberNumber && (
                    <a href={`https://viber.click/${viberNumber.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-teal-900/80 hover:bg-purple-600 flex items-center justify-center text-teal-200 hover:text-white transition-all duration-200" aria-label="Viber">
                      <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M11.97 0C5.36 0 0 5.36 0 11.97c0 2.22.61 4.3 1.68 6.1L.1 23.32c-.1.35.22.67.57.57l5.25-1.58c1.8 1.07 3.88 1.68 6.05 1.68 6.61 0 11.97-5.36 11.97-11.97S18.58 0 11.97 0zm6.9 16.32c-.31.84-1.56 1.62-2.39 1.81-.57.13-1.31.23-3.81-.8-3.2-1.32-5.26-4.57-5.42-4.78-.16-.21-1.3-1.73-1.3-3.3 0-1.57.82-2.34 1.11-2.66.29-.32.64-.4.85-.4.21 0 .42 0 .6.01.2.01.46-.08.72.54.26.62.89 2.17.97 2.33.08.16.13.35.03.56-.1.21-.15.34-.3.51-.15.17-.32.38-.46.51-.15.15-.31.31-.13.62.18.31.8 1.32 1.72 2.14 1.18 1.05 2.18 1.38 2.49 1.53.31.15.49.13.67-.08.18-.21.78-.91.99-1.22.21-.31.42-.26.7-.16.28.1 1.78.84 2.09.99.31.15.52.23.6.36.08.13.08.77-.23 1.61z"/></svg>
                    </a>
                  )}
                  {tiktokUrl && (
                    <a href={tiktokUrl} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-teal-900/80 hover:bg-black flex items-center justify-center text-teal-200 hover:text-white transition-all duration-200" aria-label="TikTok">
                      <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.82.56-1.36 1.53-1.33 2.53.01.83.43 1.64 1.11 2.12.78.56 1.8.69 2.7.4 1.02-.32 1.83-1.18 2.08-2.22.1-.5.12-1.02.12-1.53.01-4.71.01-9.42.01-14.13z"/></svg>
                    </a>
                  )}
                  {twitterUrl && (
                    <a href={twitterUrl} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-teal-900/80 hover:bg-gray-900 flex items-center justify-center text-teal-200 hover:text-white transition-all duration-200" aria-label="Twitter / X">
